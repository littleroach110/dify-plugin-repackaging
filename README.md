...
| 9 | GitHub Actions workflow exported `PIP_PLATFORM=manylinux_2_17_x86_64` into `$GITHUB_ENV`; pip reads this env var automatically and applied `--platform` to **every** pip invocation — including the "no-platform" second pass and build-dependency subprocesses — causing C-extension sdist builds to fail; the uv probe (`uv pip download --help`) was also unreliable in some Actions environments, causing unnecessary pip fallback | Renamed workflow env var from `PIP_PLATFORM` to `TARGET_PLATFORM` (avoids pip's `PIP_<OPTION>` auto-config mechanism); simplified uv probe to `command -v uv` (the `pip download` subcommand has been stable since uv 0.1.15) |
