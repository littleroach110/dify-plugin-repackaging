...
| 10 | `uv pip download --python-platform <X>` silently skips `py3-none-any` (pure-Python) wheels — they have no platform tag so uv omits them in cross-platform mode; packages like `pydantic-settings` end up absent from `./wheels/` and the runtime `uv sync` times out trying to download them from the internet | Always run two passes when cross-platform is specified: pass 1 with `--python-platform` (target binary wheels), pass 2 without (captures all `none-any` wheels and anything pass 1 missed; existing files are skipped) |
