...
| 13 | Coverage check falsely flags Windows-only packages (e.g. `cffi`, `colorama`, `pycparser` with `; sys_platform == 'win32'`) as missing when building for Linux or macOS. These packages are correctly skipped by uv/pip during download (markers are evaluated against the target platform), but the verification loop didn't evaluate markers — any package listed in `requirements.txt` with a Windows-only condition caused a false fatal error | Added platform-marker evaluation to the coverage check loop: before verifying a requirement, the `sys_platform` / `os_name` equality conditions in its marker are matched against the target OS (derived from `-p`). Windows-only (`sys_platform == 'win32'` / `os_name == 'nt'`), Linux-only, macOS-only, and POSIX-only packages are skipped when the target OS doesn't match. Packages with no marker or non-platform markers are still verified normally |
| 12 | Wheels are present in the packaged `.difypkg` but Dify's `uv sync` still downloads from the internet at install time. Three compounding causes: (a) `[tool.uv]` injection in `pyproject.toml` can fail silently for plugins with CRLF line endings — CRLF breaks awk pattern matching, causing the existing `[tool.uv]` section to be missed and a duplicate appended, which is a TOML parse error; (b) `.difyignore` entries like `wheels` (no trailing slash), `/wheels`, or `wheels/*` were not removed by the old `^wheels/` pattern, so the wheels directory could be excluded from re-packaging; (c) no fallback if `pyproject.toml` injection is bypassed by Dify | (a) Added `gsub(/\r$/, "")` in the awk injection to strip CRLF before pattern matching; (b) broadened the `.difyignore` / `.gitignore` cleanup to an ERE pattern that removes `wheels`, `wheels/`, `/wheels`, `/wheels/`, `wheels/*`, and similar — also cleans out any `uv.toml` exclusion; (c) **primary fix**: always write a `uv.toml` file in the plugin directory after downloading wheels — uv reads `uv.toml` (project-level config) with higher precedence than `[tool.uv]` in `pyproject.toml`, so `no-index = true` and `find-links = ["./wheels"]` are guaranteed to apply at runtime regardless of pyproject.toml structure |
