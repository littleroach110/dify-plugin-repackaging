...
| 10 | `uv pip download --python-platform <X>` 会静默跳过 `py3-none-any`（纯 Python）的 wheel；pip 第二遍只在第一遍失败时才运行——如果插件没有 C 扩展失败，`none-any` 包永远不会被补全 | 指定目标平台时始终运行两遍 uv；此外，在所有主下载之后始终运行 **pip 兜底 pass**（`--only-binary` + `--no-deps`，不带平台约束），确保 `none-any` 包和 sdist-only 纯 Python 包一定进入 `./wheels/` |
| 11 | 无下载后验证：某个 wheel 被静默跳过后脚本仍会打包，产生表面成功但离线安装必然失败的包 | 下载完成后增加覆盖率校验循环：逐行对比 `requirements.txt` 与 `./wheels/` 的文件列表，有缺失则立即报错终止，不再产生静默损坏的制品 |
...
| 13 | 覆盖率校验对携带平台标记的条件依赖误报为缺失。例如 `cffi`、`colorama`、`pycparser` 带有 `; sys_platform == 'win32'`，在为 Linux 打包时它们被 uv/pip 正确跳过（标记求值为 False），但校验循环没有求值标记——只要 `requirements.txt` 中存在这类条目，就会触发致命错误导致打包失败 | 在覆盖率校验循环中加入平台标记求值：读取每行标记中的 `sys_platform` / `os_name` 等式条件，与目标操作系统（由 `-p` 参数推导）对比；仅限 Windows（`sys_platform == 'win32'` / `os_name == 'nt'`）、仅限 Linux、仅限 macOS、仅限 POSIX 的包在目标系统不匹配时自动跳过；无标记或非平台标记的包仍正常校验 |
| 12 | wheel 文件已存在于打包后的 `.difypkg` 中，但 Dify 安装插件时 `uv sync` 仍然尝试从互联网下载。根因有三：(a) `pyproject.toml` 中 `[tool.uv]` 注入可能静默失败——若文件使用 Windows 换行符（CRLF），awk 模式匹配会失效，导致已有的 `[tool.uv]` 节被遗漏，最终追加重复节，引发 TOML 解析错误；(b) `.difyignore` 中形如 `wheels`（无末尾斜杠）、`/wheels`、`wheels/*` 的条目无法被旧的 `^wheels/` 模式匹配并移除，导致 wheels 目录被打包工具排除在外；(c) 若 Dify 绕过 `pyproject.toml` 注入，则无任何兜底机制 | (a) 在 awk 注入中加入 `gsub(/\r$/, "")` 以处理 CRLF 换行；(b) 将 `.difyignore`/`.gitignore` 清理模式升级为 ERE 正则，可匹配 `wheels`、`wheels/`、`/wheels`、`/wheels/`、`wheels/*` 等多种写法，并同步清除对 `uv.toml` 的排除项；(c) **核心修复**：下载完成后始终在插件目录写入 `uv.toml` 文件——uv 将项目目录下的 `uv.toml`（项目级配置）优先级设定为高于 `pyproject.toml` 的 `[tool.uv]` 节，因此 `no-index = true` 和 `find-links = ["./wheels"]` 在运行时一定生效，不再依赖 pyproject.toml 结构是否注入成功 |
